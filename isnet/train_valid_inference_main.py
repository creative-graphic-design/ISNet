...
def valid(net, valid_dataloaders, valid_datasets, hypar, epoch=0):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for i_val, data_val in enumerate(valid_dataloader):
            ...
            # wrap them in Variable
            if torch.cuda.is_available():
                inputs_val_v, labels_val_v = (
                    Variable(inputs_val.cuda(), requires_grad=False),
                    Variable(labels_val.cuda(), requires_grad=False),
                )
            else:
                inputs_val_v, labels_val_v = (
                    Variable(inputs_val, requires_grad=False),
...
if __name__ == "__main__":
    ...
    hypar["interm_sup"] = (
        False  ## in-dicate if activate intermediate feature supervision
    )
    ...
    if hypar["mode"] == "train":
        ...
        hypar["model_path"] = (
            "../saved_models/IS-Net-test"  ## model weights saving (or restoring) path
        )
        hypar["restore_model"] = (
            ""  ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
        )
        hypar["start_ite"] = (
            0  ## start iteration for the training, can be changed to match the restored training process
        )
        hypar["gt_encoder_model"] = ""
    else:  ## configure the segmentation output path and the to-be-used model weights path
        hypar["valid_out_dir"] = (
            "../your-results/"  ##"../DIS5K-Results-test" ## output inferenced segmentation maps into this fold
        )
        hypar["model_path"] = (
            "../saved_models/IS-Net"  ## load trained weights from this path
        )
        hypar["restore_model"] = (
            "isnet.pth"  ##"isnet.pth" ## name of the to-be-loaded weights
        )
    ...
    hypar["cache_boost_train"] = (
        False  ## "True" or "False", indicates wheather to load all the training datasets into RAM, True will greatly speed the training process while requires more RAM
    )
    hypar["cache_boost_valid"] = (
        False  ## "True" or "False", indicates wheather to load all the validation datasets into RAM, True will greatly speed the training process while requires more RAM
    )
    ...
    hypar["random_flip_h"] = (
        1  ## horizontal flip, currently hard coded in the dataloader and it is not in use
    )
    ...
    hypar["early_stop"] = (
        20  ## stop the training when no improvement in the past 20 validation periods, smaller numbers can be used here e.g., 5 or 10.
    )
    hypar["model_save_fre"] = (
        2000  ## valid and save model weights every 2000 iterations
    )
    ...
    hypar["max_ite"] = (
        10000000  ## if early stop couldn't stop the training process, stop it by the max_ite_num
    )
    hypar["max_epoch_num"] = (
        1000000  ## if early stop and max_ite couldn't stop the training process, stop it by the max_epoch_num
    )
